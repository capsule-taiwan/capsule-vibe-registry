...
3. 確定是要投入開發時（新專案、或**既有專案的新功能/新模組**；問問題、看程式碼、小修 bug 不算），**先問使用者是否要登記**（說明內容全公司可見；可選籠統標題保留隱私；拒絕就尊重且本 session 不再提），同意後執行：
   bash "$DIR/registry.sh" add "專案名稱" "一句話描述"
   登記單位是**專案**不是功能：新功能所屬的專案已在登記表 → 不另開一筆，改 join（還不是成員時）＋把新功能寫進 update；專案未登記 → 問的是「要不要登記這個專案」，功能寫進描述。
